<li class="header">MAIN NAVIGATION</li>
   ...
   <?php
    ...
    <?php
              $objLeaveApplication = new LeaveApplication();
              $result = $objLeaveApplication->getForDeanApproval();
              while($row = mysql_fetch_array($result))
              { 
              ?>
     <li class="treeview">
      <a href="leaveApplicant.php">
         <i class="glyphicon glyphicon-envelope" aria-hidden="true"></i>
          <span>Leave Applicant</span><span style="color:red"><?php if($row['COUNT(hodApproval)'] != 0){ ?>[ New <?php echo $row['COUNT(hodApproval)']; ?> ]<?php } ?></span>
      </a>
    </li>
    <?php
              }
